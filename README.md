...
**Das kann ich schon:**
`HTML` `CSS` `SCSS` `Bootstrap` `Emmet` `SEO` `Content-Strategie` `WordPress` `WooCommerce` `Google Analytics 4` `Search Console` `JavaScript` `Typescript` 
...
| ------- | ------------ | ----- |
| 🐴 Equestrian Game | 3D-Pferdespiel mit tiefgehender Reitmechanik, bei der Hilfengebung, Pferdeverhalten und Ausbildung ineinandergreifen. Geplant sind individuelle Pferde, Dressur und Springen, Turniersport, Stallmanagement, Zucht und eine erkundbare Spielwelt. | `3D` `Game Development` |
...
| [🗃️ Ideenarchiv](https://github.com/katharina-erfling/js_ideenarchiv) | Persönlicher digitaler Zettelkasten für mehrere tausend Storyideen mit hierarchischen Kategorien, Volltext- und Exaktsuche, Zufallsideen, Wachstumsstatistiken und einer modernen papierbasierten Oberfläche. Erster Baustein einer langfristig geplanten All-in-One-Autoren-Suite. | `JavaScript` `HTML` `CSS` `IndexedDB` |
| [📚 Memory Journal](https://github.com/katharina-erfling/js_memory-journal) | Lokale Erinnerungsbibliothek, die Fotoalbum, Scrapbook, Tagebuch und Bullet Journal verbindet. Eigene Bücher stehen in einem digitalen Bücherregal und lassen sich Seite für Seite frei mit Fotos, Texten, Collagen, Washi und weiteren Elementen gestalten. Mit lokalem Medienarchiv, Backups, Recovery sowie geplanten automatischen Rückblicken und Statistiken. | `JavaScript` `Electron` `SQLite` `HTML` `CSS` |
| [🗄️ Dokumentenarchiv](https://github.com/katharina-erfling/js_document-archive) | Persönliches lokales Dokumentenarchiv mit einem visuellen Aktenregal für Haushalt, Arbeit, Finanzen, Verträge, Bescheinigungen und weitere Unterlagen. Frei organisierbare Regalbretter und Aktenordner verbinden eine haptische Papier-/Archivoptik mit digitaler Dokumentenverwaltung. | `JavaScript` `HTML` `CSS` `Local Storage` |
| [🐾 PawLog](https://github.com/katharina-erfling/js_pawlog) | Lokale digitale Hundeakte mit Futter- und BARF-Tagebuch, dauerhaften Futterplänen, wiederverwendbaren Gerichten, Zutatenverwaltung und Gewichtsverfolgung. Unterstützt mehrere Hunde mit individuellen Profilen und Profilbildern. Die Daten werden vollständig lokal gespeichert und können per Backup exportiert und wiederhergestellt werden. | `JavaScript` `HTML` `CSS` `LocalStorage` |
...
| [🧩 CSS UI Elements](https://github.com/katharina-erfling/css_ui-elements) | Wachsende Sammlung wiederverwendbarer UI-Komponenten – Anchor Positioning Menü mit Popover API und intelligenten Fallbacks. Wird laufend erweitert. | `HTML5` `CSS3` `Anchor Positioning` `Popover API` `Work in Progress` |
...
### 🧠 UX, Design & Nutzerverhalten

| Buch | Warum es mich interessiert |
|------|---------------------------|
| **Don't Make Me Think** – Steve Krug | Der Usability-Klassiker – wer Webseiten baut, muss den kennen |
| **Laws of UX** – Jon Yablonski | Psychologische Prinzipien hinter gutem Interface-Design, kompakt auf den Punkt |
| **Storytelling im UX-Design** – Anna Dahlström | Die Brücke zwischen Content-Denken und UX – mein Kernthema |
| **Design trifft Code** – Stephanie Stimac | Wie gutes Design und sauberer Code zusammengehören |
| **UX-Strategie** – Jaime Levy | UX nicht als Ästhetik, sondern als Geschäftsstrategie denken |

### 🛒 E-Commerce & Conversion

| Buch | Warum es mich interessiert |
|------|---------------------------|
| **Warum Kunden (nicht) kaufen** – Katharina Stapel | Verhaltensökonomie trifft Marketing – warum Menschen wirklich kaufen |
| **Händlerbewertungen als Conversiontreiber** – Connor Moseler | Sehr spezifisch, sehr praxisnah – direkt anwendbar im Shop-Alltag |
